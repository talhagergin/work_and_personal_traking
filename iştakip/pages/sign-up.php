...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($email) && !empty($password) && !empty($firstName) && !empty($lastName) && !empty($username)) {
        ...
        } else {#eposta ve kullanıcı adı uygunsa
            if ($user_role == 2) {#eğer müşteri kayıtı yapıyorsa
                ...
                if (!empty($company_name) && !empty($company_email) && !empty($company_phone) && !empty($company_adress)) {
                    $get_company_name = mysqli_query($connection, "SELECT company_name FROM companies WHERE company_name LIKE '%{$company_name}%'")->fetch_assoc();
                    $get_company_email = mysqli_query($connection, "SELECT company_email FROM companies WHERE company_email LIKE '%{$company_email}%'")->fetch_assoc();

                    if (isset($get_company_name["company_name"])) {
                        $_SESSION["errors"][] = "Bu firma adı zaten sisteme kayıtlıdır.";
                        header("Location: ../pages/sign-up.php");
                        exit();
                    }
                    else if (isset($get_company_email["company_email"])) {
                        $_SESSION["errors"][] = "Bu firma epostası zaten sisteme kayıtlıdır.";
                        header("Location: ../pages/sign-up.php");
                        exit();
                    }
                    else
                    {
                        #eğer e posta ve kullanıcı adı uygunsa firma ve müşteri kayıtı yapacağız
                        ...
                        if (!$add_company) {
                            $_SESSION["errors"][] = "Firma sisteme kayıt edilemedi.";
                            header("Location: ../pages/sign-up.php");
                            exit();
                        }
                    }

                } else {#firma bilgileri eksikse
                    $_SESSION["errors"][] = "Bilgileri eksiksiz doldurunuz";
                    header("Location: ../pages/sign-up.php");
                    exit();
                }
            }
            $query2 = "INSERT INTO users (username,user_email,user_password,user_firstname,user_lastname,user_role,added_date,user_image, company_id)
                       VALUES ('{$username}','{$email}','{$password}','{$firstName}','{$lastName}','{$user_role}',now(),'{$image}', '{$connection->insert_id}')";
    ...
    } else {#eğer bilgiler kullanıcı eksikse
        $_SESSION["errors"][] = "Bilgileri eksiksiz doldurunuz";
    }
}
?>
                                ...
                                <form role="form" action="<?= $_SERVER["PHP_SELF"]; ?>" method="post">
                                    ...
                                    <div id="firma_form">
                                        ...
                                        <div class="input-group input-group-outline mb-3">
                                            ...
                                            <input type="email" name="firma_eposta" id="firma_eposta" class="form-control">
                                        </div>
                                    </div>
