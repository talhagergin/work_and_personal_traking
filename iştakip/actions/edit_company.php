...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    else if(isset($_POST["cancel_company"])){
    header("Location: ../pages/dashboard.php");
    }
}
?>
      ...
      <div class="row">
        ...
        <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" action="<?= $_SERVER["PHP_SELF"]; ?>?company_id=<?= $the_company_id=$_GET['company_id']; ?>" method="post">
                      <div class="form-group">
                        <label for="project_name" style="font-size: 20px;">Firma Ad</label>
                        <input type="text" class="form-control" placeholder="Name" name="company_name" value="<?= $company_name;?>" style="border:1px solid gray">
                      </div>
                      <div class="form-group">
                        <label for="company_name"style="font-size: 20px;">Firma Adres</label>
                        <input type="text" class="form-control" name="company_adress" id="company_adress" placeholder="Firma Adres" value="<?= $company_adress;?>"style="border:1px solid gray">
                      </div>
                      <div class="form-group">
                        <label for="company_name"style="font-size: 20px;">Firma Telefon</label>
                        <input type="text" class="form-control" name="company_phone" id="company_phone" placeholder="Firma Adres" value="<?= $company_phone;?>"style="border:1px solid gray">
                      </div>
                      <div class="form-group mb-3">
                        <label for="company_name"style="font-size: 20px;">Firma E-Posta</label>
                        <input type="text" class="form-control" name="company_email" id="company_email" placeholder="Firma Adres" value="<?= $company_email;?>"style="border:1px solid gray">
                      </div>
                      <button type="submit" name="save_company" class="btn btn-primary mr-2">Submit</button>
                      <button type="submit" name="cancel_company" class="btn btn-dark">Cancel</button>
                    </form>
                  </div>
                </div>
              </div>
